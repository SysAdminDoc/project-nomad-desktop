...
- **v6.31 — Deep Codebase Audit Round 10 (7+ commits, 647 tests pass)**:
  ...
  - **100 raw fetch→api wrapper conversions** — POST/PUT/DELETE/GET calls across 8 JS files replaced with apiPost/apiPut/apiDelete/apiFetch wrappers: workspace_memory (7), dashboard_readiness (11), ops_support (16), prep_dashboards (4), prep_ops_mapping (5), init_runtime (16), media_maps_sync (38), workspaces (3). Total api wrapper calls: 147 (was 47). 7 missing resp.ok guards on GET calls fixed. Remaining 55 raw fetch are GET calls with resp.ok guards or FormData uploads.
