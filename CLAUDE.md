...
- **SQLite** — 264 tables, WAL mode, 30s timeout, FK enforcement, SQLite backup API, 611 performance indexes
...
db.py                 # SQLite init (165 tables), organized into 6 _create_*_tables() helpers + _apply_column_migrations() + _create_indexes(). db_session() context manager
...
web/
  ...
  blueprints/         # 59 Flask blueprints: ai, agriculture, alert_rules, benchmark, brief, checklists, comms, consumption, contacts, daily_living, data_packs, disaster_modules, emergency, evac_drills, exercises, family, federation, financial, garden, group_ops, hardware_sensors, hunting_foraging, interoperability, inventory, kb, kit_builder, kiwix, land_assessment, loadout, maps, meal_planning, media, medical, medical_phase2, movement_ops, notes, nutrition, platform_security, power, preparedness, print_routes, readiness_goals, regional_profile, security, security_opsec, services, situation_room, specialized_modules, supplies, system, tactical_comms, tasks, threat_intel, timeline, training_knowledge, undo, vehicles, water_mgmt, weather
  templates/
...
## Audit History (15 rounds)
- **v7.37.0 — Premium-polish CSS coherence pass (15th polish round)**: System-level cleanup across the premium CSS stack. (1) `.btn-primary` duplicate declarations in `95_premium_polish.css` / `99_final_polish.css` collapsed — 95 now owns only the gradient fill; 99 owns border/shadow/hover stack. (2) Focus-ring tokens unified — `--premium-focus-ring` / `--premium-focus-shadow` (95) now reference `--focus-ring-halo` / `--focus-ring-color` (99) via cascade fallback for a single source of truth. (3) Added named z-index tokens to `00_theme_tokens.css` — `--z-modal-stack` (10000), `--z-app-overlay` (15000), `--z-frame-overlay` (20000), `--z-command-palette` (100000) — replaced 6 raw high-z values across 4 files. (4) Replaced remaining `transition: all` on `<progress>::-webkit-progress-value` with explicit property list. (5) Unified display-heading letter-spacing — `.home-launch-title` et al. brought from `-0.045em` down to `-0.025em` to match modal/wizard headings. (6) Extended reduced-motion block to explicitly kill entrance animations on `.modal-card` / `.wizard-card` / `.command-palette-overlay` / `.settings-modal-card` / `.shortcuts-dialog` / `.toast`. No behavior changes — style-layer only.
- **v7.36.0 — Wide-net test coverage round 2: 4 more blueprints (+ 11 real bugs fixed, 169 new tests)**: After v7.35 covered 5 untested blueprints and found 3 bugs, this release takes the same pattern further on `medical_phase2`, `meal_planning`, `threat_intel`, `tactical_comms`. Real bugs surfaced and fixed: **(1)** `meal_planning` `/api/inventory/burn-rates` referenced `cl.quantity` but the `consumption_log` column is named `amount` — **the endpoint has returned 500 on every call since it shipped**. Fixed + documented. **(2-4)** Three more PUT-schema bugs in `medical_phase2` (pregnancies, chronic conditions, vet records) where the shared `_*_SCHEMA` required `patient_name` / `condition_name` / `animal_name` → every partial update returned 400. Split into `_*_SCHEMA` / `_*_CREATE_SCHEMA` following the contacts.py pattern. **(5-11)** Seven `medical_phase2` DELETE routes (pregnancies, dental_records, herbal_remedies, chronic_conditions, vaccinations, mental_health_logs, vet_records) silently returned 200 for non-existent IDs — all now check `rowcount == 0` and return 404. New test files: `test_blueprint_medical_phase2.py` (50), `test_blueprint_meal_planning.py` (35), `test_blueprint_threat_intel.py` (38), `test_blueprint_tactical_comms.py` (46). 10 blueprints now covered (v7.35 + v7.36); 27 remain untested. Cross-blueprint smoke: 230/230 pass.
- **v7.35.0 — Wide-net test coverage pass: 5 previously-untested blueprints (+ 3 real bugs fixed)**: `CLAUDE.md` noted 37 of 60 blueprints lacked dedicated tests. This release adds full `TestXxx` suites for `alert_rules`, `vehicles`, `loadout`, `financial`, `water_mgmt` — **191 new cases, all green**. The test writing surfaced and fixed three actual bugs that had shipped: (1) `alert_rules` PUT skipped the `condition_type` + `comparison` allowlist checks that POST enforced, so stored rules could hold values that broke `evaluate()` on every tick — PUT now validates against `CONDITION_SOURCES` and `COMPARISONS`. (2) `vehicles` and `financial/barter` shared their schema between POST and PUT, which made every partial update 400-reject when the `name` / `service_type` field was omitted — split into `_VEHICLE_SCHEMA` / `_VEHICLE_CREATE_SCHEMA`, `_MAINTENANCE_SCHEMA` / `_MAINTENANCE_CREATE_SCHEMA`, `_BARTER_SCHEMA` / `_BARTER_CREATE_SCHEMA`, mirroring the pattern contacts.py already uses. (3) `alert_rules` cooldown math used deprecated `datetime.utcnow()` and failed on SQLite's `'YYYY-MM-DD HH:MM:SS'` format under Python 3.10 — now uses `datetime.now(timezone.utc)` with a space→T substitution for `fromisoformat()` compatibility. Same `utcnow()` → `_utc_now()` helper swap applied to `vehicles.py` (5 sites). Also surfaced (documented, deferred to a dedicated schema-migration pass): `water_quality_tests.coliform` declared TEXT but written as integer boolean, so `SELECT` returns the string `'0'` / `'1'` — test tolerates both representations. No regressions — full suite passes.
- **v7.34.0 — Phase C1: ICS-309 communications-log generator (ROADMAP-v9 Tier C)**: ARES/RACES deployments require an ICS-309 chronological log per operational period; NOMAD had all the traffic (`comms_log`, `lan_messages`, `mesh_messages`) but no way to emit the combined log. New `/api/print/ics-309` (HTML+JSON) merges the three sources, filters by date range (`?from`, `?to` — default last-24h UTC, matching SQLite `CURRENT_TIMESTAMP`), operator callsign (radio only), and renders the standard ICS-309 header (incident name, operational period, operator, station ID) + chronological log + signature block. New `/api/print/pdf/ics-309` mirrors the layout in ReportLab with graceful 500-fallback when the package is absent. Direction-aware from/to mapping (`rx` → from=callsign, to=SELF; `tx` → from=SELF, to=callsign). Source column distinguishes radio / LAN / mesh entries. Two new UI buttons in Settings print panel (HTML + PDF). Test suite: 9 new `TestICS309` cases (empty window, JSON shape, three-source merge, operator filter semantics, 24h window exclusion, incident/station rendering, direction flip, explicit date window, PDF graceful fallback). All 27 `test_print_documents.py` tests pass.
- **v7.33.0 — Phase A3: AI RAG scope manager (ROADMAP-v9 Tier A)**: `build_situation_context()` in `web/blueprints/ai.py` was a hardcoded waterfall over 10 tables — 90+ others were invisible to the LLM despite populated schemas. Now data-driven via new `rag_scope` table (`db.py`): each enabled row contributes one section, with configurable `weight` (emission order), `max_rows` (per-section cap), and `formatter` (`builtin` preserves original per-table output; `generic` validates table+columns against live PRAGMA and runs SELECT over a user-configured column set). 10 preexisting tables seeded as builtins with their original emission order + limits preserved verbatim. 15 additional high-value tables seeded disabled (vehicles, loadouts, water_storage, financial_reserves, preservation_batches, garden_plots, livestock, evacuation_plans, watch_schedules, waypoints, family_checkins, checklists, scheduled_tasks, comms_log, skills) — user opts in via Settings. New routes: `GET /api/ai/rag/scope`, `POST /api/ai/rag/scope` (update existing or insert custom, admin-gated, identifier-validated, `max_rows` clamped to 1..500), `DELETE /api/ai/rag/scope/<table>` (custom only; builtins must be disabled not deleted), `POST /api/ai/rag/scope/reset` (builtins only, preserves custom rows), `GET /api/ai/rag/preview` (shows exact injected payload for debugging). New self-contained JS module `_app_rag_scope.js` + compact Settings UI card. Test suite: 18 new `TestRAGScope` cases (seeding, toggle, clamping, SQL-identifier rejection, custom insert + delete, builtin-delete rejection, reset, preview shape, summary-level capping, rag_scope-missing graceful degradation). All 34 `test_blueprint_ai.py` tests pass; 61 cross-blueprint smoke tests unchanged.
- **v7.32.0 — Pass 21b: Wizard crash safety, service process cleanup, resource leaks**: Wizard daemon thread crash wrapper + re-trigger guard, ollama pull_model response FD leak fix, service process orphan prevention on DB failure (5 modules), torrent monitor thread duplication fix, portable mode symlink bypass fix (realpath), config temp file cleanup on Windows retry failure. 12 new tests (964 total).
- **v7.32.0 — Pass 21: Desktop lifecycle, DB safety, loopback hardening, federation SQLi fix**: Shutdown event for daemon threads, WAL TRUNCATE checkpoint for backups, crash-safe migrations via BEGIN IMMEDIATE, unified is_loopback_addr() across 6 callsites, CRITICAL SQL injection fix in federation sync (incoming_clocks table-name validation), os.path.basename() path traversal hardening on backup endpoints, zip extraction TOCTOU fix. 24 new tests (952 total). `npm run build` passes.
- **v7.32.0 — Deep Audit Wave 2**: PBKDF2-SHA256 credential hashing (transparent legacy upgrade on login), alert_rules cooldown datetime crash fix, AI context window system message preservation, monthly task recurrence calendar-aware math, garden seed expiration monthrange fix, version mismatch (nomad.py → Config.VERSION), coordinate validation on waypoints + rally points, triage category validation, inventory CSV import per-row error handling, 15+ innerHTML XSS escaping across system info / benchmarks / maps / models / widget config / phrase cards. 913 tests pass.
- **v7.32.0 — Security & Hardening Audit Wave 1**: Removed hardcoded VIPTrack API credentials, services bind to APP_HOST not 0.0.0.0, path traversal fix in media uploads (posixpath sanitizer), SSE event type injection prevention, WAL mode double-checked locking + PASSIVE checkpoint for backups, DB pool TOCTOU fix, zombie process cleanup in service manager, federation sync atomic commit, config save_config thread safety, frontend fetch timeouts (30s AbortController), medical vitals CHECK constraints, patient age/weight CHECK constraints, RSS feed worker error logging, rate limiter memory leak fix. 913 tests pass.
- **v7.32.0 — Validation + Auth Expansion (H2/H4)**: Contacts and inventory blueprints now carry full cerberus-style validation schemas on both POST and PUT plus `require_auth('admin')` gating on all mutating routes. `contacts.py` adds `_CONTACT_SCHEMA` (12 fields with typed max_length caps) + `_CONTACT_CREATE_SCHEMA` (name required); `POST`, `PUT`, `DELETE`, `bulk-delete`, and `import-csv` are now admin-gated. `inventory.py` replaces the prior 3-field partial schema with `_INVENTORY_SCHEMA` (17 fields — numeric bounds on quantity / min_quantity / daily_usage / cost / macros, string caps on name / location / notes / barcode); `POST`, `PUT`, `DELETE` admin-gated. Auth remains a no-op in desktop mode (localhost exempt, `NOMAD_AUTH_REQUIRED=1` to enforce). Brings validated-route count from 21 → 26 and admin-gated route count from 8 → 16. 888 tests still pass.
- **v7.31.0 — Activity Logging Completion (M2 closed)**: Activity log now captures mutations across the remaining blueprints flagged in audit M2. `supplies.py` logs 7 categories of operations (vault create/delete, skill create, ammo/community/fuel/equipment add, radiation reading + clear) — radiation events elevate to `warn` level on cumulative dose ≥ 25 REM or manual clear. `kit_builder` logs plan generation (mission + people + gaps count) and items-to-shopping-list commits. `kiwix` logs ZIM download initiation (with URL) and deletion (warn level). Three remaining blueprints flagged by M2 (`brief`, `print_routes`, `timeline`) are pure read-only — no mutations to log. 78 of 78 smoke tests still pass.
- **v7.30.0 — Pagination Completion (M1 closed)**: All list endpoints across 19 blueprints now respect `?limit=&offset=` via `get_pagination()`. This release paginated the final 7 blueprints flagged by audit M1: `consumption` (profiles), `emergency` (evac_plans), `family` (family_checkins), `meal_planning` (recipes + category filter), `alert_rules`, and `hardware_sensors` (iot_sensors, network_devices, mesh_nodes, weather_stations, gps_devices, wearable_devices, integration_configs — 7 endpoints). `specialized_modules` was already fully paginated (audit had stale info). Pre-existing unit tests (78 across smoke, family, emergency) still pass unchanged — pagination defaults (limit=100) exceed all test data sets.
- **v7.29.0 — LAN Multi-User Backbone (FTS5 + Connection Pool)**: Addresses audit findings M5 + M6.
  - **FTS5 full-text search** (`db.py:_create_fts5_tables`): five content-external FTS5 virtual tables (`notes_fts`, `inventory_fts`, `contacts_fts`, `documents_fts`, `waypoints_fts`) wired to their source tables via INSERT/UPDATE/DELETE triggers. Porter + unicode61 tokenizer with diacritic stripping. Auto-rebuild populates index on first creation only. FTS5 availability probed at startup — absence logs a warning and keeps LIKE fallback, nothing crashes.
  - **`/api/search/all` FTS-accelerated path** (`web/app.py:1063+`): detects FTS5 once per query and routes notes/inventory/contacts/documents searches through `MATCH ? LIMIT N` joins (O(log n)); conversations/skills/ammo/equipment/waypoint/frequency/patient/incident/fuel still use LIKE. Safe for arbitrary user input — query is double-quoted for FTS5 and escaped for LIKE independently.
  - **Connection pool** (`db.py:_pool_acquire/_pool_release`): process-wide `queue.Queue(maxsize=NOMAD_DB_POOL_SIZE)` (default 4). `db_session()` acquires from pool with `SELECT 1` liveness check, releases on clean exit. Dirty/in-transaction conns are closed instead of pooled. Pool invalidates on `get_db_path()` change — required for pytest's per-test `cache=shared` in-memory DB isolation. `db.pool_stats()` for diagnostics.
  - **Tests**: `tests/test_fts5_search.py` — 9 new tests covering FTS5 table creation, trigger sync on insert/update/delete, endpoint hits + special-char safety, pool stats + roundtrip. 888 of 889 existing tests pass (1 pre-existing SSE flake unrelated).
