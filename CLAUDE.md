...
- **v6.31 — Deep Codebase Audit Round 10 (7+ commits, 647 tests pass)**:
  ...
  - **89 raw fetch→api wrapper conversions** — POST/PUT/DELETE calls across 8 JS files replaced with apiPost/apiPut/apiDelete wrappers: workspace_memory (7), dashboard_readiness (8), ops_support (16), prep_dashboards (4), prep_ops_mapping (5), init_runtime (13), media_maps_sync (33), workspaces (3). Total api wrapper calls: 136 (was 47). Remaining 62 raw fetch are GET calls for data loading (lower risk).
