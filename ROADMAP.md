...
| V8-06 | **Blueprint test coverage** | ~40 of 72 blueprints lack test suites. **`roadmap_features.py`** (`89d1818`, 25 tests). **`specialized_modules.py`** (`d581cd4` + hardening `7acc471`, 45 tests — 1,794 LOC; 4 findings closed in-place). **`disaster_modules.py`** (`189099f`, 38 tests — 898 LOC). **`land_assessment.py`** (`fdaed58`, 30 tests — 465 LOC). **`kiwix.py`** (`a756f4c`, 21 tests — 137 LOC, SSRF guard + check-updates prefix-match pinned). **`brief.py`** (`tests/test_blueprint_brief.py`, 18 tests — 335 LOC, Daily Operations Brief compiler across 6 sections; **surfaced 2 real silent-failure schema mismatches**: weather section reads `temp_c`/`humidity`/`pressure`/`conditions` but table has `temp_f`/`pressure_hpa` only, and tasks section queries `title`/`due_at`/`completed_at` but table has `name`/`next_due`/`last_completed` — tasks section NEVER loads in prod due to swallowed `sqlite3.OperationalError`). Still open: `daily_living.py`, `hunting_foraging.py`, `hardware_sensors.py`, `supplies.py`, `benchmark.py`, `kb.py`, `federation.py`, `homestead.py`, and ~25 others. | **Partial — 6 of ~40 closed this cycle** |
