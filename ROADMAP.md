...
| # | Title | Description | Status |
|---|-------|-------------|--------|
| V8-04 | **innerHTML audit — newer blueprints** | 315 `innerHTML =` across 20 files. The `safeSetHTML(el, html\`\`)` + `trustedHTML` sanitization wrapper shipped as the H-05 pilot pattern. **H-17 migration progress (2026-04-24 Large-Repo-Mode session): 5 of 8 remaining tabs done** — `_tab_land_assessment.html` (25), `_tab_disaster_modules.html` (28), `_tab_medical_phase2.html` (30), `_tab_security_opsec.html` (33), `_tab_group_ops.html` (33). Remaining smallest-first: `_tab_agriculture.html` (34), `_tab_daily_living.html` (37), `_tab_specialized_modules.html` (44). L3 audit via codex-direct.sh (gpt-5.4) on the 3-tab batch returned zero findings P0/P1/P2. | Partial — 5 of 8 H-17 tabs done |
| V8-05 | ~~**Encrypt TOTP/API secrets at rest**~~ | **Done** (v7.55.0) — Fernet encrypt/decrypt with auto-generated key | Open |
| V8-06 | **Blueprint test coverage** | ~40 of 72 blueprints lack test suites. **`roadmap_features.py` covered v7.65.1-pre follow-up** (commit `89d1818`, `tests/test_blueprint_roadmap_features.py`, 25 tests across 6 resource families — recipes/batteries/warranties/ai_skills/url_monitors/personal_feeds). Still open: `agriculture.py` (covered earlier), `disaster_modules.py`, `daily_living.py`, `hunting_foraging.py`, `hardware_sensors.py`, `specialized_modules.py` and ~33 others. | Partial — 1 of ~40 closed this cycle |
...
| H-17 | **H-10 full batch** — migrate the remaining 8 tab templates to `safeSetHTML(el, html\`\`)` pattern. **5 of 8 tabs done across two sessions.** v7.65.1-pre follow-up (2026-04-24 AM) closed `_tab_land_assessment.html` (commit `0e1fb00`, 25 sites) + `_tab_disaster_modules.html` (commit `35f7b2f`, 28 sites — 16 of 28 sites closed real XSS holes where DB strings were raw-interpolated without `esc()`). Large-Repo-Mode session (2026-04-24 PM) closed `_tab_medical_phase2.html` (commit `26b3554`, 30 sites), `_tab_security_opsec.html` (33 sites — many real XSS holes closed across compartments, threats, signatures, OP log, CBRN/EMP), and `_tab_group_ops.html` (33 sites — real XSS holes across pods, roles, SOPs, duty, disputes, vote-options, ICS, CERT, damage, shelters, warnings; also hardened the vote-option onclick attribute-injection pattern via `JSON.stringify(o)` + single-quoted attribute; also renamed shadowing local `html` variable in `goLoadVotes`). L3 audit via codex-direct.sh (gpt-5.4) on the 3-tab PM batch returned zero findings P0/P1/P2. 3 tabs remain: agriculture (34), daily_living (37), specialized_modules (44). | V8-04 tail | P1 | **Partial — 5 of 8 done** |
...
### Factory-loop Large-Repo-Mode session (2026-04-24 PM)

Single-session run under Large-Repo-Mode (auto-engaged: 70K+ web/ LOC, 488 tracked files, 105 test files, 5 open V8). 1 iteration, 3 atomic per-task commits, no release. L3 audit dispatched via `codex-direct.sh` (gpt-5.4) per recipe v0.5.1+ — real cross-family signal restored; zero findings P0/P1/P2 on the 3-tab aggregate. Declared degradations: L4 counter-audit + U2/T2 counter-passes skipped (same-model duplication has no signal value in single-session); Copilot CLI delegation for bulk implementation not feasible from this VSCode-extension context (recipe honest-fallback). S-phase (AI-scrub) still parks 4 historical CoAuthored-By commits from v7.7.0–v7.10.0 era — destructive without explicit `--manual-scrub` opt-in.

Commits landed on top of `e4f5a35`:
- `26b3554` V8-04 / H-17: migrate `_tab_medical_phase2.html` (30 sites). 50/50 test_blueprint_medical_phase2 + 63/63 vitest green.
- (next) V8-04 / H-17: migrate `_tab_security_opsec.html` (33 sites). 25/25 test_blueprint_security + test_security_v2 + 63/63 vitest green.
- (latest) V8-04 / H-17: migrate `_tab_group_ops.html` (33 sites). 10/10 test_blueprint_group_ops + 63/63 vitest green.

H-17 now at 5 of 8. Next session should target smallest-first remainder (agriculture 34 → daily_living 37 → specialized_modules 44).
