...
| V8-09 | **Split create_app()** | **Partial** (v7.57.0) — `web/middleware.py` (339 L, CSRF + rate limit + host validation + auth guard + LAN auth + security headers + DB cleanup + `_setup_error_handlers`) and `web/blueprint_registry.py` (237 L, 72-blueprint registry) already shipped. `create_app()` now 1,243 lines (down from 1,655). What remains: 50+ inline page routes (`/`, `/home`, `/situation-room`, etc.) → extract to `web/pages.py` blueprint; 4 background-thread setups (discovery listener, auto-backup scheduler trio, SSE cleanup) → extract to `web/background.py`. Tracked as H-14 for a dedicated iteration. | Partial |
...
| H-03 | **Narrow `except Exception` in Situation-Room fetch workers** — 9 sites narrowed in iter 1 (64 → 55 bare-except). Iter 2 target: another 10-15 sites in sanctions / UCDP / cyber / yield-curve / stablecoins workers. Iter 3: remaining non-finally sites. | Known Remaining Issue | P1 | **Open (partial)** |
...
| H-06 | **`get_db()` try/finally scan** — automated AST walker surfaces every `get_db()` call that doesn't pair with a `try/finally` or `db_session()`. First pass produces a ranked list in `docs/db-leak-audit.md`; subsequent iterations convert. Iter 2 delivers the scan tool + report. | Known Remaining Issue | P1 | Open |
...
| H-08 | **V8-09 split `create_app()`** — 1,363 lines → `web/middleware.py` + `web/blueprint_registry.py` + `web/error_handlers.py`. Keep `create_app()` a thin orchestrator. Iter 2 deliverable. | V8-09 | P0 | Open |
| H-09 | **V8-11 lazy blueprint registration** — depends on H-08; iter 3 deliverable. | V8-11 | P1 | Open |
...
| H-11 | **AST test for future `_start_lock` regressions** — parallel to H-02's manager atomicity pin, add a test asserting `services.ollama.start()` body contains a `with _start_lock:` block and that the body's first statement inside it is the `is_running() and check_port()` short-circuit check. Prevents a future edit from moving the guard above the lock. | Iter 1 audit carry-over | P2 | Open |
| H-12 | **Rename `trustedHTML` → `markTrusted`** — audit MED finding: `trustedHTML` reads as "safe" to uninformed callers and is easy to over-use. Rename to `markTrusted(s)` + keep a 1-line shim for one version. | Iter 1 audit carry-over | P2 | Open |
| H-13 | **`_parse_feed` malformed-input test** — iter 1 audit LOW: `UnicodeDecodeError`, `LookupError`, truncated XML. Add 3-4 `tests/test_blueprint_situation_room.py` assertions that feed `_parse_feed` adversarial payloads and verify the worker returns `[]` without raising. | Iter 1 audit carry-over | P2 | Open |
| H-14 | **create_app() final thinning** — extract inline page routes (`/`, `/home`, `/situation-room`, …) into `web/pages.py` blueprint + 4 background-thread setup trios into `web/background.py`. Target: create_app() ≤ 300 lines. V8-09 Partial carry-over; deserves a dedicated iteration because the shared closures (`_render_workspace_page`, `_get_template_i18n_context`, `_safe_json_value`, `_discovery_listener_started` module lock) make a naive extract risky. | V8-09 carry-over | P1 | Open |
| H-15 | **`get_db()` automated leak audit tool** — AST walker delivered in iter 2 as `tools/audit_db_sessions.py`; produces ranked `docs/db-leak-audit.md`. First pass output drives the iter 3 conversion work. | H-06 deliverable | P1 | Open |

### Iteration cadence (3-iteration target run)

- **Iter 1 → v7.62.0** — **Shipped.** H-01 / H-02 / H-04 / H-05 / H-07 Done. H-03 partial (9/64 sites). ROADMAP updated with H-11/H-12/H-13 audit carry-overs.
- **Iter 2 → v7.63.0** — H-08 (V8-09 split), H-10 batch (_tab_land_assessment.html — 25 sites), H-06 scan + report, H-03 continuation (+10-15 sites).
- **Iter 3 → v7.64.0** — H-09 (V8-11 lazy blueprints), H-10 batch 2 (7 remaining tabs), H-03 final narrowing, H-11/H-12/H-13 audit carry-overs.
