...
| V8-06 | **Blueprint test coverage** | ~40 of 72 blueprints lack test suites. **`roadmap_features.py`** (25 tests). **`specialized_modules.py`** (45 tests + 4 findings closed). **`disaster_modules.py`** (38 tests). **`land_assessment.py`** (30 tests). **`kiwix.py`** (21 tests, SSRF + prefix-match pinned). **`brief.py`** (22 tests + **2 silent-failure bugs surfaced & FIXED**: weather schema-alignment + tasks query `name/next_due/last_completed` migration; payload backwards-compat preserved via legacy aliases). **`benchmark.py`** (`tests/test_blueprint_benchmark.py`, 20 tests — 538 LOC; concurrent-run 409 guards verified for both /run and /network; 32MB real disk I/O on /storage; ollama mocked for /ai-inference; pinned a contract surprise — /network default peer is loopback but the validator rejects loopback so the default is never actually used). Still open: `daily_living.py`, `hunting_foraging.py`, `hardware_sensors.py`, `supplies.py`, `kb.py`, `federation.py`, `homestead.py`, and ~24 others. | **Partial — 7 of ~40 closed this cycle** |
