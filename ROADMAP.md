...
|---------|-------|----------|--------------------|--------------------|
| [Crosstalk-Solutions/project-nomad](https://github.com/Crosstalk-Solutions/project-nomad) | ~23.7k | Upstream (Docker) | Polished onboarding wizard, content-pack marketplace UI, one-click Docker deploy, wider community & YouTube ecosystem | Content-pack download UI comparable to an app store; guided first-run experience with progress tracking |
| [grocy/grocy](https://github.com/grocy/grocy) | ~9.0k | Household ERP | Barcode-to-product lookup via OpenFoodFacts, recipe-driven consumption (auto-deduct ingredients), chore scheduling with user assignment, battery replacement tracker, full REST API with Swagger UI docs | Recipe-driven auto-consumption, product database lookup, chore/maintenance scheduler with assignees |
| [Homebox (sysadminsmedia)](https://github.com/sysadminsmedia/homebox) | ~5k+ | Home Inventory | QR code label printing per item, warranty & maintenance schedule tracking, receipt/document attachments per item, ultra-light resource usage (~50 MB RAM), OIDC SSO | QR label generation per inventory item, warranty tracking fields, maintenance reminders per asset |
| [glanceapp/glance](https://github.com/glanceapp/glance) | ~33.6k | Dashboard | YAML-based widget config (no code), clean card-based layout with consistent spacing, RSS/Reddit/HN/GitHub/weather widgets with minimal setup, responsive mobile-first design | Declarative widget configuration file, cleaner card density on home page |
...
| [meshtastic/web](https://github.com/meshtastic/web) | ~750 | Mesh Comms | Real-time node map with signal quality, message threading, channel management UI, Bluetooth/serial/HTTP transport selection, position sharing | Direct Meshtastic serial integration (NOMAD has stub), signal quality visualization |

### Key Gaps Identified

1. **Onboarding & First-Run** — No guided setup wizard; new users face a wall of tabs
...
| P1-06 | **Collapsible sidebar groups** | Let users collapse sidebar group headers (OVERVIEW, INTEL, etc.) to reduce visual noise | Dashy |
...
| P1-09 | **Toast action buttons** | Add "Undo" action button on delete toasts (leverage existing undo system) | Homebox |
...
| P2-03 | **QR code label generation** | Generate printable QR code labels for inventory items linking to their detail page; batch print sheet | Homebox |
| P2-04 | **Recipe-driven consumption** | Add recipes that reference inventory items; "Cook this" button auto-deducts ingredient quantities | Grocy |
| P2-05 | **Equipment maintenance scheduler** | Recurring maintenance reminders for generators, vehicles, water filters, etc. with overdue alerts | Homebox, Prepper Nerd |
| P2-06 | **Drag-and-drop widget reorder** | Let users drag widgets on the home page to reorder; persist layout to localStorage/DB | Dashy, Glance |
...
| P2-08 | **Expanded i18n coverage** | Increase translation keys from 56 to 200+ covering all UI labels, button text, and error messages | Homepage (40+ languages) |
...
| P2-11 | **Content pack browser** | Dedicated UI for browsing available data packs (ZIMs, maps, books) with search, categories, size, and one-click install | Upstream NOMAD |
| P2-12 | **Service health history graph** | Track service up/down over time; show sparkline uptime graph on each service card | Dashy, Homepage |
| P2-13 | **Inline survival quick-reference** | Bundle a curated offline survival reference (water purification, fire, shelter, first aid) as searchable built-in cards instead of external ZIM dependency | SurvivalManual |
...
| P2-15 | **Inventory item photos gallery** | Grid view of all inventory photos with lightbox zoom; filter by category | Homebox |
| P2-16 | **Map bookmark/favorite locations** | Star frequently-used map locations for quick jump; show in sidebar | Upstream NOMAD |
| P2-17 | **Notification center panel** | Unified notification drawer (slide-out) aggregating alerts, task due dates, expiring items, service events | Homepage, Glance |
| P2-18 | **CSV export for all entities** | One-click CSV export button on every list view (contacts, medical, tasks, etc.), not just inventory | Grocy |

### P3: Nice-to-Haves and Polish

| # | Title | Description | Inspired By |
|---|-------|-------------|-------------|
| P3-01 | **Animated page transitions** | Subtle slide/fade transitions between tabs instead of instant swap | Dashy |
...
| P3-04 | **Command palette** | Ctrl+K opens a VS Code-style command palette for jumping to any section, running actions, searching | Dashy |
| P3-05 | **Customizable status strip** | Let users choose which metrics appear in the top status strip | Glance |
| P3-06 | **Meshtastic serial bridge** | Real Meshtastic integration via serial/USB with node map, message display, and channel config | Meshtastic Web |
| P3-07 | **Offline plant identification** | Bundled lightweight ML model for plant ID from camera photos | SurvivalManual |
| P3-08 | **Insurance & warranty tracker** | Track warranties, insurance policies, and important document expiry dates with reminders | Homebox, Prepper Nerd |
| P3-09 | **Chore/task assignment** | Assign scheduled tasks to specific family members/contacts; track completion per person | Grocy |
| P3-10 | **Battery replacement tracker** | Track batteries in devices (flashlights, radios, etc.) with replacement date reminders | Grocy |
| P3-11 | **Visual alert rule builder** | Drag-and-drop UI for building compound alert rules (backend already supports it) | Internal backlog |
| P3-12 | **Plugin/extension API** | Define hook points and a simple plugin manifest so community can add custom tabs/routes | Dashy |
| P3-13 | **Tauri shell alternative** | Replace pywebview with Tauri for smaller binary, faster startup, and native feel | Internal backlog |
| P3-14 | **SBOM generation** | Generate Software Bill of Materials on each release for supply-chain transparency | Internal backlog |
| P3-15 | **Regional content packs** | Pre-configured data bundles for Canada (ECCC), UK (Met Office), EU (Copernicus), Australia (BOM) | IIAB |
...
| U-17 | **Situation Room initial load** — 34 fetch workers fire simultaneously on tab open | Prioritize above-the-fold cards; lazy-load below-fold cards on scroll into view |
