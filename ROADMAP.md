...
| H-17 | **H-10 full batch** — migrate the remaining 8 tab templates (`specialized_modules` 44, `daily_living` 37, `agriculture` 34, `security_opsec` 33, `group_ops` 33, `medical_phase2` 32, `disaster_modules` 28, `land_assessment` 25) to `safeSetHTML(el, html\`\`)` pattern. Deferred past iter 3 because most already use a per-tab `esc()` helper that mitigates XSS — the migration is primarily a consistency/grep-discoverability win rather than a security fix. Pick up in a dedicated batch iteration. | V8-04 tail | P2 | Open |
...
- **V8 roadmap**: 16 of 24 items Done, 1 Partial (V8-09), 6 Open (V8-02 / V8-03 / V8-04 / V8-06 / V8-08 / V8-11), 1 Blocked (V8-21 — Apple Developer credentials).
