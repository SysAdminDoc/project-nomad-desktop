...
| V8-06 | **Blueprint test coverage** | ~40 of 72 blueprints lack test suites. **`roadmap_features.py`** covered v7.65.1-pre follow-up (commit `89d1818`, 25 tests across 6 families). **`specialized_modules.py`** covered 2026-04-24 late-PM (`tests/test_blueprint_specialized_modules.py`, 38 tests across 14 resources — supply caches, pets+food-status, youth, eol, procurement+budget-summary, intel, fabrication, badges+seed, awards+leaderboard, calendar+upcoming, legal+expiring, drones, flights+stats, fitness+stats, content packs — 1,794 LOC / 60+ routes). Surfaced 4 follow-up findings: fitness POST lacks 400 guard, flights/stats shape inconsistent, `/drones/<id>/flights` frontend↔backend wiring gap, fitness frontend↔DB field-name mismatch (`workout_type` vs `exercise_type`). Still open: `disaster_modules.py`, `daily_living.py`, `hunting_foraging.py`, `hardware_sensors.py`, and ~33 others. | **Partial — 2 of ~40 closed this cycle** |
