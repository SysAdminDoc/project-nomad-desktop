...
def api_security_zones():
    db = get_db()
    rows = db.execute('SELECT id, name, zone_type, camera_ids, waypoint_ids, alert_on_entry, alert_on_exit, threat_level, color, notes FROM perimeter_zones ORDER BY name LIMIT 200').fetchall()
    db.close()
    result = []
    for r in rows:
        entry = dict(r)
