...
class TestAlertRulesPayloadValidation:
    ...
    def test_create_rejects_non_object(self, client):
        resp = client.post(
            '/api/alert-rules',
            data='[]',
            content_type='application/json',
    ...
    def test_create_rejects_wrong_shape(self, client):
        cases = [
            {'name': 123},
            {'condition_type': []},
        ...
        for payload in cases:
            resp = client.post('/api/alert-rules', json=payload)
            assert resp.status_code == 400, payload
            assert resp.get_json()['error'] == 'Validation failed'

    def test_update_rejects_malformed_json(self, client):
        resp = client.put(
            '/api/alert-rules/1',
            data='{bad',
            content_type='application/json',
    ...
    def test_update_rejects_non_object(self, client):
        resp = client.put(
            '/api/alert-rules/1',
            data='"string"',
            content_type='application/json',
    ...
    def test_update_rejects_wrong_shape(self, client):
        resp = client.put('/api/alert-rules/1', json={'severity': 123})
        assert resp.status_code == 400
        assert resp.get_json()['error'] == 'Validation failed'
