...
**H2. Input Validation Coverage — 9 of 1,628 Routes** — **Partial (v7.32.0)**
...
- Contacts + inventory fully covered (v7.32.0): `_CONTACT_SCHEMA` (12 fields, name required on create) applied to POST + PUT; `_INVENTORY_SCHEMA` (17 fields incl. quantity/min_quantity/daily_usage numeric bounds, cost cap, macro bounds) replaces the prior 3-field partial schema on POST + PUT.
- Remaining: medical (Phase 9), group_ops, security_opsec, agriculture, all Phase 17-20 blueprints. ~15 more routes high-priority, ~1500 low-priority.
...
**H4. No Auth Middleware Wired to Routes** — **Expanded in v7.32.0**
...
- Coverage: financial (8 routes — v7.28.0); contacts POST/PUT/DELETE/bulk-delete/import-csv (5 routes — v7.32.0); inventory POST/PUT/DELETE (3 routes — v7.32.0). 16 total mutating routes gated.
- Remaining: medical, group_ops, security_opsec, sensitive admin endpoints (backups/restore, self-test, factory reset).
...
**M1. 19 Blueprints Return Unbounded Query Results** — **Complete (v7.30.0)**
...
- v7.30.0 closes the gap: `consumption`, `emergency`, `family`, `meal_planning` (recipes), `hardware_sensors` (7 list endpoints — iot_sensors, network_devices, mesh_nodes, weather_stations, gps_devices, wearable_devices, integration_configs), `alert_rules`. `specialized_modules` was already paginated (audit stale). No separate `callshield` / `alerts` blueprints exist — the audit entry conflated the AlertRules blueprint (now done) and a would-be callshield module (never shipped).

**M2. 11 Blueprints Have No Activity Logging** — **Complete (v7.31.0)**
- v7.27.0: `contacts`, `vehicles`. v7.28.0: `checklists`, `weather`. v7.31.0: `supplies` (7 mutation types logged — vault create/delete, skill create, ammo/community/fuel/equipment add, radiation reading + clear), `kit_builder` (plan generation, items-to-shopping), `kiwix` (zim_download_started, zim_deleted). `brief`, `print_routes`, `timeline` are pure read-only blueprints (zero mutating routes) and correctly have no activity logging — the audit count of 11 was inclusive of read-only modules.
...
**M5. No FTS5 Full-Text Search** — **Fixed in v7.29.0**
- Added `_create_fts5_tables()` in db.py. Five content-external FTS5 virtual tables (notes_fts, inventory_fts, contacts_fts, documents_fts, waypoints_fts) with INSERT/UPDATE/DELETE triggers keeping them synced to base tables. Porter unicode61 tokenizer with diacritic stripping. Auto-rebuild on first creation only. `/api/search/all` detects FTS5 availability at query time and uses `MATCH` joins for notes/inventory/contacts/documents (waypoints remains LIKE until next iteration). O(log n) vs O(n) on those hot paths.

**M6. No Connection Pooling** — **Fixed in v7.29.0**
- Added bounded process-wide SQLite connection pool (`queue.Queue(maxsize=NOMAD_DB_POOL_SIZE)`, default 4). `db_session()` acquires from pool with `SELECT 1` validation and releases on clean exit; on exception or open transaction the connection is closed rather than pooled. Pool invalidates itself when `get_db_path()` changes (ensures test isolation across in-memory DBs). Opt-out by setting `NOMAD_DB_POOL_SIZE=0`. Diagnostics via `db.pool_stats()`.
